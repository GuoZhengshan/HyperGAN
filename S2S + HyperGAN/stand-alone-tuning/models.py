...
#       self.regul = torch.sum(rela_embed**2)
    ...
    3-arity
    """
    def forward_tri(self, facts, arch, trainer):
        ...
        if self.args.loss == "binCrossEntropy":
            ...
            e1_scores = trainer.merge_confidence_weight(facts, e1_label, 1, e1_scores)
            e2_scores = trainer.merge_confidence_weight(facts, e2_label, 2, e2_scores)
            e3_scores = trainer.merge_confidence_weight(facts, e3_label, 3, e3_scores)
    ...
    4-arity
    """
    def forward_quad(self, facts, arch, trainer):
        ...
        if self.args.loss == "binCrossEntropy":
            ...
            e2_scores = trainer.merge_confidence_weight(facts, e2_label, 2, e2_scores)
            e3_scores = trainer.merge_confidence_weight(facts, e3_label, 3, e3_scores)
            e4_scores = trainer.merge_confidence_weight(facts, e4_label, 4, e4_scores)
            ...
            loss1 = -4 * pos_trip + max_neg_e1 + torch.log(torch.sum(torch.exp(neg_e1 - max_neg_e1), 1)) +\
                   max_neg_e2 + torch.log(torch.sum(torch.exp(neg_e2 - max_neg_e2), 1)) +\
                   max_neg_e3 + torch.log(torch.sum(torch.exp(neg_e3 - max_neg_e3), 1)) +\
