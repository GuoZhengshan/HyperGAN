...
#                        self.best_struct = [item.index(True) for item in arch.tolist()]
    ...
    def train(self, train_data, valid_data, tester_val, tester_tst, tester_trip=None):
        ...
        self.trainer = HyperGAN(train_data, self.args, self.model, self.n_ent, self.n_rel)
        ...
        for epoch in range(n_epoch):
            if epoch % self.args.classify_steps == 0:
                print("Train HyperGAN")
                self.trainer.train_HyperGAN(self.trainer, self.args.epochs)
                self.trainer.test_ave_score(self.trainer)
            ...
            for facts in batch_by_size(n_batch, train_data, n_sample=n_train):
                ...
                if self.n_arity == 3:
                    ...
                    loss = self.model.forward_tri(facts, self.op_idx, self.trainer)
                    loss.backward()

                elif self.n_arity == 5:
                    loss = self.model.forward_quad(facts, self.op_idx, self.trainer)
                    loss.backward()
