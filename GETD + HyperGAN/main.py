...
class Experiment:
    ...
    def get_er_vocab(self, data, miss_ent_domain):
        ...
        if len(data[0])-1 == 3:     # arity = 3
            if miss_ent_domain == 1:
                for triple in data:
                    er_vocab[(triple[0], triple[2], triple[3])].append(triple[1])       # <class 'list'>, {(0, 2, 3): [e1，e2]}
    ...
    def evaluate(self, model, data, W):
        ...
        for miss_ent_domain in range(1, ary+1):
            ...
            for i in range(0, len(test_data_idxs), self.batch_size):
                ...
                # data_batch1 = np.array(test_data_idxs[i: i + self.batch_size])等价与上面data_batch
                ...
                for j in range(data_batch.shape[0]):
                    ...
                    filt = er_vocab[tuple(er_vocab_key)]    # 该tuple对应的真实e的idx，是list（可能是多个fact）
                    # 先把这条test data对应的pred值取出来，是float
                    target_value = pred[j, e_all_idx[miss_ent_domain-1][j]].item()      # model算出来的value
                    # 把filt 对应的多个pred值置为0，为了排除其他fact影响该条数据data
                    pred[j, filt] = 0.0
                    # 再把这条test data对应的pred值赋回去，如果filt的size = 1，其实上面三句话没有作用意思
                    pred[j, e_all_idx[miss_ent_domain-1][j]] = target_value

                sort_values, sort_idxs = torch.sort(pred, dim=1, descending=True)   # 降序排
                sort_idxs = sort_idxs.cpu().numpy()
                for j in range(data_batch.shape[0]):        # batch
    ...
    def train_and_eval(self, args):
        ...
        self.entity_idxs = {d.entities[i]: i for i in range(len(d.entities))}       # 字典：r --> idx
        self.relation_idxs = {d.relations[i]: i for i in range(len(d.relations))}

        train_data_idxs = self.get_data_idxs(d.train_data)      # 格式为 [(), (), ()]
        ...
        for miss_ent_domain in range(1, ary+1):
            er_vocab = self.get_er_vocab(train_data_idxs, miss_ent_domain)      # {(除缺失e以外的所有): 缺失的e}
            er_vocab_pairs = list(er_vocab.keys())
            er_vocab_list.append(er_vocab)
            er_vocab_pairs_list.append(er_vocab_pairs)
        ...
        for it in range(0, self.num_iterations):
            ...
            for miss_ent_domain in range(1, ary+1):
                er_vocab = er_vocab_list[miss_ent_domain-1]     # dict
                er_vocab_pairs = er_vocab_pairs_list[miss_ent_domain-1]     # list
                ...
                for j in range(0, len(er_vocab_pairs), self.batch_size):
                    ...
                    pred = pred.to(device)     # batch * len(E)
                    loss = model.loss(pred, label)      # binary_cross_entropy
                    loss.backward()
                    opt.step()
